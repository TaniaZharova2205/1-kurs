...
Window canvas(Display* dpy) {
	...
	attr.event_mask = (ButtonPressMask | KeyPressMask | ExposureMask | StructureNotifyMask | FocusChangeMask); /* Event mask */
	scr =  DefaultScreen(dpy);
	w = DisplayWidth(dpy, scr) / 2;
	h = DisplayHeight(dpy, scr) / 2;
	x = w / 2; y = h / 2;
	win = XCreateWindow(dpy, DefaultRootWindow(dpy), x, y, w, h, 1, DefaultDepth(dpy, scr), InputOutput, CopyFromParent, (CWOverrideRedirect | CWBackPixel | CWEventMask), &attr);
...
int regraph(Display* dpy, Window win, GC gc, int NoFillFace) {
	...
	for(i=0; i<NVERT; i++)                    /* redraw even vertex */
		XFillArc(dpy, win, gc, vertex[i].x - (VDOT>>1), vertex[i].y - (VDOT>>1), VDOT, VDOT, 0, (64*360));
	return(0);
} /* regraph */
...
int reface(Display* dpy, Window win, GC gc, int f) {
	...
	for(i=0; i< face[f].Cn; i++)            /* redraw inner face's vertex */
		XFillArc(dpy, win, gc, face[f].top[i].x - (VDOT/2), face[f].top[i].y - (VDOT/2), VDOT, VDOT, 0, (64*360));
	return(0);
} /* reface */
